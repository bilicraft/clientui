...
public class BilicraftPlayerRender {
     ...
     * 展示玩家皮肤的模型
     */
    private static final ModelBiped steveModel = new ModelBiped();
    ...
    /**
     * 玩家模型头部俯仰角(沿x轴旋转)
     * 负数朝上看,正数朝下看,单位为角度,一般范围是[-90, 90]
     */
    private float headPitch = 0f;

    /**
     * 玩家模型头部旋转角(沿y轴旋转)
     * 负数朝左看,正数朝右看,单位为角度,一般范围是[-90, 90]
     */
    ...
    /**
     * 玩家模型行走速度
     * 范围是[0.0, 1.0],0.0为原地站立不动,1.0为正常行走
     */
    ...
    public void setSkin(String name) {
        ...
        if (resourceLocation == null) {   //此处为net.minecraft.util下的StringUtils
     ...
     * @param roll  横滚角
     */
    public void setHeadLook(float pitch, float yaw, float roll) {
        headPitch = pitch;
        headYaw = yaw;
        headRoll = roll;
    }
